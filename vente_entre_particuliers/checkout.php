          ...
          <div class="card-body">
            ...
            <?php foreach ($items as $it): ?>
              ...
              <div class="d-flex gap-3 align-items-center py-2 border-bottom">
                ...
                <div class="flex-grow-1">
                  ...
                  <?php if (!$canBuy): ?>
                    <div class="text-danger small"><i class="bi bi-exclamation-triangle"></i> Pas disponible en paiement direct.</div>
                  <?php elseif (!$stockOk): ?>
                    <div class="text-danger small"><i class="bi bi-exclamation-triangle"></i> Stock insuffisant.</div>
                  <?php endif; ?>
                </div>
            ...
            <div class="mt-3">
              ...
              <div class="d-flex justify-content-between">
                ...
                <div class="fw-semibold" id="frais-display"><?php echo number_format($frais_livraison, 2); ?> DH</div>
              </div>
              <hr>
              <div class="d-flex justify-content-between align-items-center">
                <div class="text-muted">Total</div>
                <div class="fs-4 fw-bold" id="total-display"><?php echo number_format($total, 2); ?> DH</div>
              </div>
            </div>
          ...
          <div class="card-body">
              ...
              <?php if ($mode_reception === "LIVRAISON"): ?>
                <div class="mt-3">
                  ...
                  <form method="GET" class="d-flex gap-2">
                    ...
                    <select class="form-select" name="ville" id="ville-select">
                    ...
                    <button class="btn btn-outline-secondary" type="submit" id="ville-ok-btn">OK</button>
                  </form>
            ...
            <?php else: ?>
              <form action="actions/stripe_create_session.php" method="POST">
                ...
                <input type="hidden" name="ville_livraison" id="hidden-ville" value="<?php echo htmlspecialchars($ville_livraison); ?>">
                <input type="hidden" name="frais_livraison" id="hidden-frais" value="<?php echo htmlspecialchars((string)$frais_livraison); ?>">
...
(function () {
  ...
  if (!select) return; // pas en mode livraison

  const SELLER_CITY = <?php echo json_encode((string)$sellerCity, JSON_UNESCAPED_UNICODE); ?>;
  const FEE_SAME = <?php echo (float)$sellerFeeSame; ?>;
  const FEE_OTHER = <?php echo (float)$sellerFeeOther; ?>;
  const SUBTOTAL = <?php echo (float)$subtotal; ?>;

  const fraisEl = document.getElementById("frais-display");
  const totalEl = document.getElementById("total-display");
  const hiddenVille = document.getElementById("hidden-ville");
  const hiddenFrais = document.getElementById("hidden-frais");
  const okBtn = document.getElementById("ville-ok-btn");

  // Plus besoin du bouton OK : le calcul se fait au changement
  ...
  function recalc() {
    ...
    const sameCity = ville.trim().toLowerCase() === SELLER_CITY.trim().toLowerCase();
    const frais = sameCity ? FEE_SAME : FEE_OTHER;
    const total = SUBTOTAL + frais;

    if (fraisEl) fraisEl.textContent = money(frais);
    if (totalEl) totalEl.textContent = money(total);
    if (hiddenVille) hiddenVille.value = ville;
    if (hiddenFrais) hiddenFrais.value = frais;
  }
