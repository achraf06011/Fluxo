...
// En ouvrant les messages, on marque les notifs de message comme lues
// => le total de la cloche (notifications) diminue.
markNotificationsRead($pdo, $userId, ["NEW_MESSAGE"]);
      ...
      <div class="card shadow-sm border-0">
        <div class="card-body">
          ...
          <?php else: ?>
            ...
            <div id="thread-box" class="border rounded p-3" style="height: 420px; overflow:auto; background:#fafafa;">
              <div id="thread-empty" class="text-muted" style="<?php echo count($thread) === 0 ? "" : "display:none;"; ?>">
                Aucun message. Écris le premier.
              </div>
              <?php foreach ($thread as $m): ?>
                <?php $mine = ((int)$m["id_expediteur"] === $userId); ?>
                <div class="d-flex <?php echo $mine ? "justify-content-end" : "justify-content-start"; ?> mb-2" data-id="<?php echo (int)$m["id_message"]; ?>">
            ...
            <form id="msg-form" class="mt-3" action="actions/message_action.php" method="POST">
              <input type="hidden" name="id_annonce" value="<?php echo (int)$annonceId; ?>">
              <input type="hidden" name="to" value="<?php echo (int)$toId; ?>">

              <div class="input-group">
                <textarea id="msg-input" class="form-control" name="contenu" rows="2" placeholder="Écrire un message..." <?php echo $canChat ? "" : "disabled"; ?>></textarea>
...
<?php if ($annonceId > 0 && $toId > 0): ?>
<?php
  $lastMsgId = 0;
  foreach ($thread as $m) {
    $lastMsgId = max($lastMsgId, (int)$m["id_message"]);
  }
?>
<script>
(function () {
  const ANNONCE_ID = <?php echo (int)$annonceId; ?>;
  const TO_ID = <?php echo (int)$toId; ?>;
  const CAN_CHAT = <?php echo $canChat ? "true" : "false"; ?>;
  let lastId = <?php echo (int)$lastMsgId; ?>;
  ...
  function buildBubble(m) {
    ...
    wrap.className = "d-flex " + (m.mine ? "justify-content-end" : "justify-content-start") + " mb-2";
    wrap.dataset.id = m.id_message;

    const bubble = document.createElement("div");
    bubble.style.maxWidth = "78%";
    bubble.className = "p-2 px-3 rounded " + (m.mine ? "bg-dark text-white" : "bg-white border");

    const date = document.createElement("div");
    date.className = "small " + (m.mine ? "text-white-50" : "text-muted");
    date.textContent = (m.date_envoi || "").substring(0, 16);

    const body = document.createElement("div");
    body.style.whiteSpace = "pre-wrap";
    body.textContent = m.contenu; // textContent => pas d'injection HTML
  ...
  function appendMessages(list) {
    ...
    list.forEach(function (m) {
      if (box.querySelector('[data-id="' + m.id_message + '"]')) return;
      box.appendChild(buildBubble(m));
      if (m.id_message > lastId) lastId = m.id_message;
      added = true;
    });
    if (added) {
  ...
  async function poll() {
    try {
      const res = await fetch(
        "messages_poll.php?annonce=" + ANNONCE_ID + "&to=" + TO_ID + "&after=" + lastId,
  ...
  if (form && CAN_CHAT) {
    form.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        fd.append("id_annonce", ANNONCE_ID);
        fd.append("to", TO_ID);
        fd.append("contenu", contenu);

        const res = await fetch("message_send_ajax.php", { method: "POST", body: fd });
        const data = await res.json();

        if (data.ok && data.new_message) {
          appendMessages([data.new_message]);
          input.value = "";
        } else {
