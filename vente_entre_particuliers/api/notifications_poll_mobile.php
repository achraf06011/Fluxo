...
try {
    ...
    // Compteurs non lus par catégorie (messages / ventes / commandes)
    $counts = [
        "messages" => 0,
        "ventes" => 0,
        "commandes" => 0,
    ...
    $stmtCounts = $pdo->prepare("
        SELECT type_notification, COUNT(*) AS c
        FROM notification
        WHERE id_user = ?
          AND COALESCE(is_read, 0) = 0
        GROUP BY type_notification
    ");
    $stmtCounts->execute([$userId]);

    foreach ($stmtCounts->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $type = strtoupper(trim((string)($row["type_notification"] ?? "")));
        $c = (int)$row["c"];
        ...
        if ($type === "NEW_MESSAGE") {
            ...
            $counts["ventes"] += $c;
        } elseif (in_array($type, ["ORDER_UPDATE", "ORDER_DELIVERED", "ORDER_STATUS"], true)) {
            $counts["commandes"] += $c;
        }
    }
