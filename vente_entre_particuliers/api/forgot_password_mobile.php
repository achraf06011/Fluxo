...
try {
    ...
    // Lien WEB vers reset_password.php (fiable, ouvrable depuis le navigateur du téléphone).
    // On utilise l'hôte appelé par l'app (ex: 192.168.1.13) pour que le lien soit joignable
    // sur le même wifi — les schémas "fluxomobile://" ne marchent pas dans Expo Go.
    $scheme = (!empty($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] !== "off") ? "https" : "http";
    $host = $_SERVER["HTTP_HOST"] ?? "localhost";
    // SCRIPT_NAME = /.../vente_entre_particuliers/api/forgot_password_mobile.php
    // on remonte du dossier /api/ vers la racine du site
    $siteBase = rtrim(dirname(dirname($_SERVER["SCRIPT_NAME"] ?? "")), "/\\");
    $resetLink = $scheme . "://" . $host . $siteBase . "/reset_password.php?token=" . urlencode($token);
