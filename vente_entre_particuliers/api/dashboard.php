...
try {
  ...
  // Stats supplémentaires : vues, annonces actives, favoris reçus, note moyenne
  try {
    $stmt = $pdo->prepare("SELECT COALESCE(SUM(nb_vues), 0) FROM annonce WHERE id_vendeur = ?");
    $stmt->execute([$userId]);
    $stats["total_vues"] = (int)($stmt->fetchColumn() ?: 0);

    $stmt = $pdo->prepare("SELECT COUNT(*) FROM annonce WHERE id_vendeur = ? AND statut = 'ACTIVE'");
    $stmt->execute([$userId]);
    $stats["annonces_actives"] = (int)($stmt->fetchColumn() ?: 0);
  } catch (Exception $e) {}
  ...
  try {
    ...
    ");
    $stmt->execute([$userId]);
    $stats["favoris_recus"] = (int)($stmt->fetchColumn() ?: 0);
  } catch (Exception $e) {}

  try {
    $stmt = $pdo->prepare("SELECT COALESCE(AVG(note), 0) AS m, COUNT(id_review) AS n FROM review WHERE id_seller = ?");
    $stmt->execute([$userId]);
    $r = $stmt->fetch();
    if ($r) {
      $stats["note_moyenne"] = round((float)($r["m"] ?? 0), 1);
      $stats["nb_avis"] = (int)($r["n"] ?? 0);
    }
  } catch (Exception $e) {}
