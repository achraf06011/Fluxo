...
import { getRecentlyViewed, RecentItem } from "../../utils/recently-viewed";
...
export default function HomeScreen() {
  ...
  const [recommended, setRecommended] = useState<AnnonceType[]>([]);
  ...
  async function loadExtras() {
    // Récemment vues : stockage local
    const rv = await getRecentlyViewed();
    setRecentlyViewed(rv);

    // Recommandé pour toi : API (si connecté)
    try {
      const user = await getUser();
      if (user?.id_user) {
        const res = await fetch(
          `${API_BASE}/api/recommended_mobile.php?user_id=${Number(
            user.id_user
          )}`
        );
        const data = await res.json();
        setRecommended(
          data.ok && Array.isArray(data.annonces) ? data.annonces : []
        );
      } else {
        setRecommended([]);
      }
          ...
          <View style={styles.headerWrap}>
            ...
            {!isFiltering && recentlyViewed.length > 0 ? (
              <View style={styles.sectionBlock}>
                <Text style={styles.sectionHeading}>Récemment vues</Text>
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                ...
                >
                  {recentlyViewed.map((rv) => (
                    <TouchableOpacity
                      key={`rv-${rv.id_annonce}`}
                      style={styles.miniCard}
                      activeOpacity={0.9}
                      onPress={() => router.push(`/annonce/${rv.id_annonce}`)}
                    >
                      <Image
                        source={{
                          uri: rv.cover_url || "https://picsum.photos/300",
                        }}
            ...
            {!isFiltering && recommended.length > 0 ? (
              <View style={styles.sectionBlock}>
                <Text style={styles.sectionHeading}>Recommandé pour toi</Text>
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                  contentContainerStyle={styles.miniScroll}
