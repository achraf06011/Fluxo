...
import { API_BASE, WEB_BASE } from "@/config/api";
...
export default function AnnonceDetailsScreen() {
  ...
  async function loadAnnonce() {
    try {
      ...
      if (data.annonce) {
        addRecentlyViewed({
          id_annonce: Number(data.annonce.id_annonce),
          titre: String(data.annonce.titre ?? ""),
          prix: data.annonce.prix,
          ancien_prix: data.annonce.ancien_prix ?? null,
          ville: data.annonce.ville ?? null,
          mode_vente: data.annonce.mode_vente ?? null,
          cover_url: data.annonce.cover_image_url ?? null,
        });
      }
    } catch (error: any) {
